...
GLboolean testBoxSphereIntersection(SBB sbb, AABB two) // AABB - Circle collision
{
    
    // Calculate AABB info (center, half-extents)
    glm::vec3 aabb_center = glm::vec3((two.max + two.min) / 2.0f);
    glm::vec3 aabb_half_extents = glm::vec3((two.max - two.min) / 2.0f);
    glm::vec3 difference = sbb.center - aabb_center;
    ...
    difference = closest - sbb.center;
    return glm::length(difference) < sbb.ratio;
}     
...
bool testBoxBoxIntersection(AABB aabb1, AABB aabb2) {
	if (aabb1.min.x + (aabb1.max.x - aabb1.min.x) >= aabb2.min.x &&
		aabb1.min.x <= (aabb2.min.x + (aabb2.max.x - aabb2.min.x)) &&
		aabb1.min.y + (aabb1.max.y - aabb1.min.y) >= aabb2.min.y &&
		aabb1.min.y <= (aabb2.min.y + (aabb2.max.y - aabb2.min.y)) &&
		aabb1.min.z + (aabb1.max.z - aabb1.min.z) >= aabb2.min.z &&
		aabb1.min.z <= (aabb2.min.z + (aabb2.max.z - aabb2.min.z)))
		return true;
	else
		return false;
}
