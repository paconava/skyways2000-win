 ...
 * @param TextureTarget Tipo de textura que ser� objetivo para este objeto
...
bool CubemapTexture::Load() {
  ...
  // Tipo de interpolaci�n de mapeo de la textura a los trigangulos.
  ...
  for (unsigned int i = 0; i < ARRAY_SIZE_IN_ELEMENTS(types); i++) {
    ...
    if (format == -1) {
      std::cout << "No se encontr� la imagen: " << fileNames[i].c_str()
                << " - Abortando." << std::endl;
      exit(-1);
    }
...
/**
 * Este m�todo hace el enalce del objeto de textura con su unidad de textura.
 * De acuerdo al dise�o de la arquitectura puede haber diferentes objetos de textura
 * para un m�delo, si se desea enlazar una unidad de textura a un objeto de textura digamos A
